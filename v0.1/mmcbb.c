...
#define SELECT()	PORTB &= ~_BV(PB2)	/* CS = L */
#define	DESELECT()	PORTB |=  _BV(PB2)	/* CS = H */
#define	SELECTING	!(PORTB &  _BV(PB2))	/* CS status (true:CS low) */
#define	FORWARD(d)	xmit(d)				/* Data forwarding function (console out) */

#define MMC_SD_PORT       PORTB                    
#define MMC_SD_CS_PIN     PB2     //CS = PB2
...
//#define rcvr_spi_m(dst)	SPDR=0xFF; while(!(SPSR & _BV(SPIF))); *(dst)=SPDR
...
/* old version ... try to speed it up
...
)
{
	...
	if (token != 0xFD) {	/* Is data token */
		...
		xmit_spi(0xFF);
		resp = rcv_spi();				/* Reveive data response */
